...
def evaluate(test_data_file, indexs_lb, device, fea, clf, tokenizer):
    ...
    auc = roc_auc_score(labels_1d, logits_1d)
...
for seed in range(42, 47):
    set_seed(seed)
    source_projects = ['FFmpeg','LibPNG', 'LibTIFF', 'VLC']
    target_projects = ['FFmpeg','LibPNG', 'LibTIFF', 'VLC']
    ...
    for source_project in source_projects:
        ...
        for target_project in target_projects:
            ...
            log_filename = './log/ALstage_%s_%s_seed%d.log' % (source_project, target_project, seed)
            ...
            logger.info('='*100)
            logger.info('New Cross-Project Combination Started')
            logger.info('Source Project: %s', source_project)
            logger.info('Target Project: %s', target_project)
            logger.info('Random Seed: %d', seed)
            logger.info('Log File: %s', log_filename)
            logger.info('='*100)
            
            for ratio in np.arange(0.1, 0.15, 0.1):
                ...
                logger.info(f"运行时间为: {run_time} 秒")
